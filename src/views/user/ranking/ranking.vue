...
<template>
  <div class="wa-ranking">
    <checker class="wa-checkbox" v-model="active" radio-required default-item-class="checkbox-item" selected-item-class="checkbox-item-selected">
      <!--<checker-item v-if="item" :value="item.name" v-for="(item, index) in games" :key="index">{{ item.name }}</checker-item>-->
...
export default {
  ...
  methods: {
    ...
    compareDown (propertyName) { // 降序排序
      // if ((typeof this.rankList[0][propertyName]) !== 'number') { // 属性值为非数字
